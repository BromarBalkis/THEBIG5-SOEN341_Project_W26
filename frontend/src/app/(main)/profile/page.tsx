...
export default function ProfilePage() {
  ...
  const [selectedDiets, setSelectedDiets] = useState<string[]>(currentUser?.dietaryPreferences || [])
          ...
          {DIETARY_OPTIONS.map((option) => (
            <label
              ...
              className={`cursor-pointer flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors ${
                selectedDiets.includes(option) ? 'border-primary bg-primary-light' : 'border-gray-200'
              }`}
            >
              <input
                type="checkbox"
        ...
        {allergies.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {allergies.map((allergy) => (
              <div key={allergy} className="flex items-center gap-1 bg-primary-light text-primary-dark px-3 py-1 rounded-full text-sm font-medium">
                {allergy}
                <button
